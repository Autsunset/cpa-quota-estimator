...
- 绘制周额度进度预测：对比实际用量、按时间均匀消耗的可持续线和当前平均速度投影，并显示时间进度、每日消耗速度、预计耗尽时间、重置时预计用量、下次重置时间及倒计时。
...
绿色平均线表示恰好在重置时使用 100% 的可持续节奏；实际线高于平均线代表消耗偏快。该投影按当前周累计平均速度计算，会随之后的使用强度变化而更新。
