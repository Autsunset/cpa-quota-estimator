...
- 绘制周额度进度预测：对比实际用量、按时间均匀消耗的可持续线、本人本周累计平均线和最近 24 小时速度投影，并显示时间进度、每日消耗速度、预计耗尽时间、重置时预计用量、下次重置时间及倒计时。
...
周额度进度预测使用本周累计额度比例与已经过时间计算：

```text
时间进度       = 已经过秒数 / 周窗口总秒数
当前平均速度   = 当前已用百分比 / 已经过天数
近期消耗速度   = 最近约 24 小时额度增长 / 对应时间
重置时预计用量 = 当前已用百分比 / 时间进度
预计耗尽时间   = 窗口开始时间 + 已经过时间 × 100 / 当前已用百分比
```

绿色平均线表示恰好在重置时使用 100% 的可持续节奏；紫色线表示本周累计平均速度，并延伸到预计 100% 耗尽点（若本周不会耗尽，则延伸到重置时预计用量）；橙色线按最近约 24 小时的额度增长速度预测，近期样本不足时回退到本周累计平均速度。实际线高于绿色线代表消耗偏快。
