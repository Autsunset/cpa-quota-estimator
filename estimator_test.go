...
func TestPublicReleaseDefaults(t *testing.T) {
	...
	for _, marker := range [][]byte{
		...
		[]byte("Spark 月度额度汇总"),
		[]byte("spark_summary"),
		[]byte("spark_weekly_quota"),
		[]byte("spark_weekly_summary"),
		[]byte("spark_five_hour_quota_detected"),
		[]byte("renderSparkWeeklyQuota"),
		[]byte("sparkWeeklyCurve"),
		[]byte("sparkWeeklyCycleRange"),
		[]byte("sparkWeeklyCycleRows"),
		[]byte("sparkWeeklyMonthlyIDs"),
		[]byte("sparkFiveHourMonthlyTitle"),
		[]byte("Spark 独立双轴额度"),
		[]byte("weeklyQuota"),
		[]byte("weekly_quota"),
		[]byte("weekly_summary"),
		[]byte("five_hour_quota_detected"),
